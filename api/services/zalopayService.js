...
const createZaloPayOrder = async (amount, orderId, userId) => {
    try {
        console.log('DEBUG - ZaloPay service received amount:', amount);
        const transID = Math.floor(Math.random() * 1000000);
        const embed_data = {
            redirecturl: process.env.CLIENT_URL ? `${process.env.CLIENT_URL}/account/bookings` : 'http://localhost:5173/account/bookings'
        };
        const items = [];
        ...
        const order = {
            ...
            app_user: `user_${userId}_order${orderId}`, // No underscore before order (following reference)
            app_time: Date.now(),
        ...
        // Convert data to string before hashing (following reference format)
        ...
        order.mac = CryptoJS.HmacSHA256(dataStr, zalopayConfig.key1).toString(CryptoJS.enc.Hex);
